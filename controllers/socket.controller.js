...
export default class SocketController {
  static async onConnection(socketHandler, incomingMessage) {
    ...
    const existingData = JSON.parse(
      readFileSync(`./data/${incomingMessage}.json`, (err, data) => {
        return data;
      })
    );
  ...
  static async onRelay(socketHandler, incomingMessage, roomID) {
    ...
    var infodata = JSON.parse(
      readFileSync("./data/info.json", (err, data) => {
        return data;
      })
    );
    ...
    if (infodata.available.includes(formID)) {
      formDatabase = JSON.parse(
        readFileSync(`./data/${formID}.json`, (err, data) => {
          return data;
        })
      );
      for (var iterable = 0; iterable < formDataKeys.length; iterable++) {
        // console.log(iterable, formDatabase[`question${iterable}`])
        formDatabase[`question${iterable}`]["choices"][
          formData[formDataKeys[iterable]]
        ] += 1;
      }
    }
  ...
  static async onNotLive(socketHandler, roomID) {
    socketHandler.to(roomID).emit("stop", "Not Live");
  }

  static async onLive(socketHandler, roomID) {
    socketHandler.to(roomID).emit("start", "Live");
  }

  static async onAbort(socketHandler, roomID) {
    socketHandler.to(roomID).emit("abort", "Abort");
  }

  static async onReload(socketHandler, roomID) {
    socketHandler.to(roomID).emit("reload", "Reload");
  }
}
